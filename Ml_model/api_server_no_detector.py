...
# from anomaly_detector import NetworkAnomalyDetector # Disabled for no-detector version
import json
...
# Initialize detector
# detector = NetworkAnomalyDetector() # Disabled
...
def health():
    return jsonify({
        ...
        'service': 'ML Anomaly Detection API (NO DETECTOR)'
    })
...
def analyze_flow():
    ...
    Wrapper version that does NOT use the detector.
    Always returns BENIGN.
    """
    try:
        data = request.json
...
if __name__ == '__main__':
    print("Starting ML Anomaly Detection API (NO DETECTOR) on http://0.0.0.0:5000")
